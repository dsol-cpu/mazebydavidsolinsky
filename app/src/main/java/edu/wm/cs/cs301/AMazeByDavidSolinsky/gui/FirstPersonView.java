...
import edu.wm.cs.cs301.AMazeByDavidSolinsky.generation.BSPBranch;
import edu.wm.cs.cs301.AMazeByDavidSolinsky.generation.BSPLeaf;
import edu.wm.cs.cs301.AMazeByDavidSolinsky.generation.BSPNode;
...
public class FirstPersonView {
	 ...
	 * @param g for drawing on the buffer image
	 ...
	 * 
	 */
	public void draw(MazePanel g, int x, int y, int walkStep, int ang, float percentToExit) {
	...
	private int blend(int c0, int c1, double weight0) {
		...
		if (weight0 > 0.95)
			return c0;
	    double r = weight0 * Color.red(c0) + (1-weight0) * Color.red(c0);
