...
import edu.wm.cs.cs301.AMazeByDavidSolinsky.FinishActivity;
 ...
 * dealt with a key listener (SimpleKeyListener) and then handed over to a
 * Controller object by way of the keyDown method.
 * 
 * Responsibilities: Show the first person view and the map view, Accept input
 * for manual operation (left, right, up, down etc), Update the graphics,
...
//	Controller control;
	...
	private CompassRose cr = new CompassRose(); // compass rose to show current direction
	 ...
	 *                   drawing
	 */
	public void start(MazePanel panel, RobotDriver driver) {
		started = true;
		...
		if (panel != null) {
			...
			// run drive2exit
			try {
				System.out.println("Drive2Exit started");
				driver.drive2Exit();
			} catch (Exception e) {
	 ...
	 * simple key listener calls this method to communicate input. Method requires
	 * {@link #start(MazePanel, RobotDriver) start} to be called before.
	 * 
	 * North is 0,-1, South is 0,1
	 * West is -1,0   East is 1,0
...
//				control.switchFromPlayingToWinning(0);
		...
		case Right: // turn right
			System.out.println("Before turning right: " + this.getCurrentDirection());
			rotate(-1);
			System.out.println("After turning right: " + this.getCurrentDirection());
			break;
	...
	/**
	 * Calculates a distance to exit as a percentage. 1.0 is for the starting
	 * position as this is the maximal distance possible.
	 * 
	 * @return a value between 0.0 and 1.0, the smaller the closer
	 */
	float getPercentageForDistanceToExit() {
		return mazeConfig.getDistanceToExit(px, py) / ((float) mazeConfig.getMazedists().getMaxDistance());
	}
	...
	/**
	 * Sets fields dx and dy to be consistent with current setting of field angle.
	 */
	...
	synchronized private void rotate(int dir) {
		...
		System.out.println("Initial direction set: " + "dx: " + dx + ", dy: "+ dy);
		setDirectionToMatchCurrentAngle();
		System.out.println("Post direction set: " + "dx: " + dx + ", dy: "+ dy);
...
//			cr.paintComponent(panel);
	...
	/**
	 * Checks if the current position and direction faces a dead end
	 * 
	 * @return true if at the current position there is a wall to the left, right
	 *         and front, false otherwise
	 */
	private boolean isFacingDeadEnd() {
		return (!isOutside(px, py) && mazeConfig.hasWall(px, py, getCurrentDirection())
	...
	/*
	 * private void dbg(String str) { //System.out.println(str); }
	 * 
	 * private void logPosition() { if (!deepdebug) return;
